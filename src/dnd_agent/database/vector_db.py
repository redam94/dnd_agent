...
class PostgresVectorManager:
    ...
    def store_campaign_info(
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING id
            """,
                (campaign_id, info_type, title, content, Json(metadata or {}), embedding),
            )
            result = cur.fetchone()
            self.conn.commit()
    ...
    def store_chat_message(
        ...
        with self.conn.cursor() as cur:
            cur.execute(
                ...
                RETURNING id
            """,
                (campaign_id, session_id, role, content, Json(metadata or {}), embedding),
            )
            result = cur.fetchone()
            self.conn.commit()
            return result[0] if result else None
    ...
    def get_chat_history(
        self, campaign_id: str, session_id: Optional[str] = None, limit: int = 50
    ) -> List[Dict[str, Any]]:
    ...
    def search_chat_history(
        self, campaign_id: str, query: str, session_id: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
