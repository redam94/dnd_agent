...
class EnhancedCodeAnalyzer:
    ...
    def _analyze_node(
        self, node, source: str, file_path: Path, parent: Optional[str] = None, depth: int = 0
    ):
    ...
    def _process_class(
        self, node: ast.ClassDef, source: str, file_path: Path, parent: Optional[str]
    ):
...
class BatchAIDocGenerator:
    ...
    def generate_batch_documentation(
        self, elements: List[CodeElement], style: DocStyle
    ) -> Dict[str, str]:
        ...
        logger.info(
            f"Processing {len(elements_to_process)} elements (skipped {len(elements) - len(elements_to_process)} cached/complete)"
        )
...
"""
    ...
    def _parse_batch_response(
        self, response: str, batch: List[CodeElement]
    ) -> Dict[CodeElement, str]:
...
class DocumentationReportGenerator:
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        avg_complexity = (
            sum(e.complexity for e in self.elements.values()) / total_elements
            if total_elements > 0
            else 0
        )
        ...
        report = {
            "summary": {
                ...
                "coverage_percentage": (
                    (documented / total_elements * 100) if total_elements > 0 else 0
                ),
            },
            "by_type": by_type,
            "complexity": {
    ...
    def _save_markdown_report(self, report: Dict, output_path: Path):
        ...
        for type_name, stats in report["by_type"].items():
            ...
            content.append(
                f"- {type_name}: {stats['documented']}/{stats['total']} ({coverage:.1f}%)"
            )
        ...
        for element in report["complexity"]["high_complexity_elements"]:
            content.append(
                f"- {element['name']} ({element['type']}): complexity={element['complexity']} in {element['file']}"
            )
...
class MarkdownDocumentationBuilder:
    ...
    def build_documentation(
        self, elements: Dict[str, CodeElement], enhanced_docs: Dict[str, str]
    ) -> Dict[str, Path]:
    ...
    def _generate_api_docs(
        self, elements: Dict[str, CodeElement], enhanced_docs: Dict[str, str], api_dir: Path
    ) -> Dict[str, Path]:
        """Generate API documentation."""
        api_files = {}
        ...
        for key, element in elements.items():
            module_path = (
                element.file_path.split("::")[0] if "::" in element.file_path else element.file_path
            )
    ...
    def _generate_module_doc(
        self, module_path: str, elements: list, enhanced_docs: Dict[str, str], output_path: Path
    ):
        ...
        if classes:
            ...
            content.append("")
            for key, element in classes:
                doc = enhanced_docs.get(
                    element.hash, element.docstring or "No documentation available."
                )
    ...
    def _generate_mkdocs_config(self):
        ...
        config = {
            ...
            "theme": {"name": "material", "palette": {"primary": "indigo", "accent": "indigo"}},
            "nav": [
                {"Home": "README.md"},
                {"API Reference": "api/index.md"},
                {
    ...
    def _generate_docusaurus_config(self):
        ...
        config = {
            "docs": [
                {"type": "doc", "id": "README"},
                {"type": "category", "label": "API", "items": ["api/index"]},
                {
                    "type": "category",
                    "label": "Guides",
                    "items": ["guides/installation", "guides/quickstart", "guides/development"],
                },
            ]
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Advanced AI Documentation Generator with Markdown Support"
    )
    ...
    parser.add_argument(
        "--markdown-style",
        choices=["github", "mkdocs", "sphinx_md", "docusaurus"],
        default="github",
        help="Markdown documentation style",
    )
