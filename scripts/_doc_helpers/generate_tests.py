...
class CodeInspector:
    ...
    ) -> Tuple[Dict[str, ClassInfo], Dict[str, FunctionSignature]]:
...
class TestStrategy:
    ...
    def determine_test_types(
        func: FunctionSignature, class_info: Optional[ClassInfo] = None
    ) -> List[TestType]:
...
class AITestGenerator:
    ...
    def _generate_function_test(
        self, func: FunctionSignature, test_type: TestType
    ) -> Optional[TestCase]:
...
"""
    ...
    def _create_method_test_prompt(
        self, class_info: ClassInfo, method: FunctionSignature, test_type: TestType
    ) -> str:
...
class TestCoverageAnalyzer:
    ...
    def analyze_coverage(self) -> Dict[str, Any]:
        ...
        for module_path in source_modules:
            ...
            if test_files:
                ...
                module_coverage["has_fixtures"] = (
                    "@pytest.fixture" in test_content or "setUp" in test_content
                )
            else:
    ...
    def generate_coverage_badge(self, coverage_percentage: float) -> str:
        ...
        color = (
            "red" if coverage_percentage < 50 else "yellow" if coverage_percentage < 80 else "green"
        )
...
def main():
    ...
    for module in modules:
        try:
            ...
            suite = test_gen.generate_test_suite(
                str(module.relative_to(source_dir.parent)), classes, functions
            )
    ...
    if args.coverage:
        ...
        # Generate badge
        badge_svg = analyzer.generate_coverage_badge(
            coverage_report["summary"]["coverage_percentage"]
        )
